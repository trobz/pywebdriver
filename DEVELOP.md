...
On weak CPUs without AVX support (e.g. Intel Celeron J4125), the prebuilt `pycups` wheel
may fail at runtime. Fix it by rebuilding `pycups` from source in the venv:
...
`debian/rules`), so a build produced by CI or `build_ubuntu_2604.sh` works on these CPUs
out of the box.
